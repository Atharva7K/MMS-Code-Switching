...
WAV2VEC2_KWARGS_DOCSTRING = r"""
            ...
            max_length (`int`, *optional*):
                Controls the maximum length to use by selone of the truncation/padding parameters.
...
class Wav2Vec2CTCTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.unrequired_tokens_mask = None
    ...
    def set_code_switched_target_langs(self, target_lang_1, target_lang_2):
        if self.target_lang_1 is not None and self.target_lang_1 == target_lang_1:
            return
        
        if self.target_lang_2 is not None and self.target_lang_2 == target_lang_2:
            return
        
        if target_lang_1 not in self.vocab:
            raise ValueError(f"{target_lang_1} does not exist. Choose one of {', '.join(self.vocab.keys())}.")
        if target_lang_2 not in self.vocab:
            raise ValueError(f"{target_lang_2} does not exist. Choose one of {', '.join(self.vocab.keys())}.")
...
class Wav2Vec2Tokenizer(PreTrainedTokenizer):
    ...
    def _convert_token_to_id(self, token: str) -> int:
        """Converts a token (str) in an index (integer) using the vocab."""
        print(self.encoder.get(token, self.encoder.get(self.unk_token)))
        return self.encoder.get(token, self.encoder.get(self.unk_token))
