...
#         self.act_fn = nn.GELU()
#         self.linear_1 = nn.Linear(self.hidden_dim_in, self.hidden_dim_in // 2)
#         self.linear_2 = nn.Linear(self.hidden_dim_in // 2, self.hidden_dim_in // 4)
#         self.linear_3 = nn.Linear(self.hidden_dim_in // 4, self.input_dim)
#         self.linear_4 = nn.Linear(self.input_dim, self.hidden_dim_in // 4)
#         self.linear_5 = nn.Linear(self.hidden_dim_in // 4, self.hidden_dim_in // 2)
        
#     def forward(self, hidden_states: torch.FloatTensor):
#         hidden_states = self.norm(hidden_states)

#         hidden_states = self.linear_1(hidden_states)
#         hidden_states = self.act_fn(self.dropout(hidden_states))
...
#         hidden_states = self.act_fn(self.dropout(hidden_states))
#         hidden_states = self.linear_5(hidden_states)
...
#         super().__init__()
#         self.dropout = nn.Dropout(0.3)
#         self.input_dim = config.adapter_attn_dim * 2
#         self.hidden_dim_in = config.hidden_size * 2
#         self.hidden_dim_out = config.hidden_size
...
class TransformerCodeSwitcher(nn.Module):
    ...
    def forward(
        ...
        print('boooom', hidden_states)
        outputs = (hidden_states,)

        if output_attentions:
            outputs += (attn_weights,)

        print('Output shape for code switcher output', outputs[0])
        return outputs
...
#             self.code_switcher = Wav2Vec2PreAdapterSwitching()
...
class Wav2Vec2EncoderLayerStableLayerNormForCodeSwitching(nn.Module):
    ...
    def forward(
        ...
        if self.adapter_layer_1 is not None and self.adapter_layer_2 is not None:
                print('CS sequence', code_switch_sequence)
...
class Wav2Vec2EncoderStableLayerNorm(nn.Module):
    ...
    def forward(
        ...
        print('code_switch_sequence', code_switch_sequence)
        deepspeed_zero3_is_enabled = is_deepspeed_zero3_enabled()
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def _get_code_switched_adapters(self):
        if self.config.do_code_switching is False:
            raise ValueError(f"{self.__class__} has no code switched adapter layers. Make sure to define `config.adapter_attn_dim`.")
        ...
        for name, module in self.named_modules():
            if isinstance(module, Wav2Vec2AttnAdapterLayer):
                if 'adapter_layer_1' in name:
                    for param_name, param in module.named_parameters():
                        adapter_weights_1[".".join([name, param_name])] = param
                else:
                    if 'adapter_layer_2' in name:
                        for param_name, param in module.named_parameters():
    ...
    def load_adapters_for_code_switching(self, target_lang_1 = 'ara', target_lang_2 = 'eng', force_load=True, **kwargs):

        if self.config.do_code_switching is False:
            raise ValueError(f"Cannot load code switching adapters if `config.do_adapter_switching` is set to False.")

        if self.target_lang_1 == target_lang_1 and self.target_lang_2 == target_lang_2 and not force_load:
            logger.warn(f"Code switched adapter weights are already set to {target_lang_1} and {target_lang_2}.")
            return
        ...
        # 1. Let's first try loading a safetensors adapter weight
        if use_safetensors is not False:
            filepath1 = WAV2VEC2_ADAPTER_SAFE_FILE.format(target_lang_1)
            filepath2 = WAV2VEC2_ADAPTER_SAFE_FILE.format(target_lang_2)
            try:
                weight_path_1 = cached_file(
                    model_path_or_id,
                    filename=filepath1,
                    force_download=force_download,
                    resume_download=resume_download,
                ...
                state_dict_1 = safe_load_file(weight_path_1)
                state_dict_2 = safe_load_file(weight_path_2)
                ...
                if use_safetensors:
                    raise EnvironmentError(
                        ...
                        " from 'https://huggingface.co/models', make sure you don't have a local directory with the"
                        f" same name. Otherwise, make sure '{model_path_or_id}' is the correct path to a"
                        f" directory containing a file named {filepath1} or {filepath2}."
                    )
                
        if state_dict_1 is None and state_dict_2 is None:
            filepath_1 = WAV2VEC2_ADAPTER_PT_FILE.format(target_lang_1)
            filepath_2 = WAV2VEC2_ADAPTER_PT_FILE.format(target_lang_2)
            ...
            try:
                ...
                state_dict_2 = torch.load(weight_path_2, map_location="cpu")
        ...
        elif len(missing_keys_1) > 0:
            raise ValueError(f"The adapter weights {weight_path_1} has missing keys: {', '.join(missing_keys_1)}.")
    
        if len(unexpected_keys_2) > 0:
            raise ValueError(f"The adapter weights {weight_path_2} has unexpected keys: {', '.join(unexpected_keys_2)}.")
...
class Wav2Vec2ForCTCWithAdapterSwitching(Wav2Vec2PreTrainedModel):
    def __init__(self, config, target_lang_1: Optional[str] = 'ara', target_lang_2: Optional[str] = 'eng'):
        super().__init__(config)
